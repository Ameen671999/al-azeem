...
export class OrderCreateComponent implements OnInit, OnDestroy {
  ...
  options: string[] = [];
  result = ["Kindly update invalid entries:\n"];
  ...
  getRadioButton(value) {
    ...
    if (this.radioCount > 1 && this.radioButtonVal_array.pop() != value) {
        ...
        for (let i = 0; i < ar.length; i++) {
          for (let j = 0; j < itemsAr.length; j++) {
            if (itemsAr[j].itemName == ar[i].unititem) {
              ...
              //console.log("selectedIndex_itemsAr", selectedIndex_itemsAr);
      ...
      //console.log(this.updatedOrder_Data_Array);
  ...
  onSubmit() {
    ...
    if (this.result.length > 1) {
      ...
      this.result = ["Kindly update invalid entries:\n"];
    }
